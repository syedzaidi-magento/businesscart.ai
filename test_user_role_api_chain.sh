...
# Step 2: Associate Customer with Company
echo "2. Associating customer user (${USER_IDS[customer]}) with company ($COMPANY_ID)..."
...
# Step 3: List Products for Customer
echo "3. Listing products for customer (${USER_IDS[customer]})..."
CUSTOMER_PRODUCTS_RESPONSE=$(curl -s -w "\n%{http_code}" -X GET "$PRODUCT_API/products" \
  -H "Authorization: Bearer ${JWTS[customer]}")
CUSTOMER_PRODUCTS_STATUS=$(echo "$CUSTOMER_PRODUCTS_RESPONSE" | tail -n1)
CUSTOMER_PRODUCTS_BODY=$(echo "$CUSTOMER_PRODUCTS_RESPONSE" | sed -e '$d')
if [ "$CUSTOMER_PRODUCTS_STATUS" -eq 200 ]; then
  echo "Products for customer:"
  echo "$CUSTOMER_PRODUCTS_BODY" | jq .
else
  echo -e "${RED}Error fetching products for customer: HTTP $CUSTOMER_PRODUCTS_STATUS${NC}"
  exit 1
fi

# Step 4: Create and List Orders for Customer
echo "4. Finding order for customer (${USER_IDS[customer]})..."
CUSTOMER_ORDER_RESPONSE=$(curl -s -w "\n%{http_code}" -X GET "$ORDER_API/orders" \
  -H "Authorization: Bearer ${JWTS[customer]}")
CUSTOMER_ORDER_STATUS=$(echo "$CUSTOMER_ORDER_RESPONSE" | tail -n1)
CUSTOMER_ORDER_BODY=$(echo "$CUSTOMER_ORDER_RESPONSE" | sed -e '$d')

if [ "$CUSTOMER_ORDER_STATUS" -eq 200 ] && [ "$(echo "$CUSTOMER_ORDER_BODY" | jq '. | length')" -gt 0 ]; then
  echo "$CUSTOMER_ORDER_BODY" | jq .
  CUSTOMER_ORDER_ID=$(echo "$CUSTOMER_ORDER_BODY" | jq -r '.[0]._id // .[0].id // empty')
  if [ -z "$CUSTOMER_ORDER_ID" ]; then
    echo -e "${RED}Error: Failed to extract order ID from response${NC}"
    exit 1
  fi
  echo -e "${GREEN}Order found for customer. Order ID: $CUSTOMER_ORDER_ID${NC}"
else
  echo -e "${RED}No order found. Creating new order for customer...${NC}"
